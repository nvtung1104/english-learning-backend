<template>
  <AdminPageLayout title="Cấp độ" subtitle="Quản lý cấp độ khóa học">
    <template #actions>
      <AppButton variant="primary" size="md" @click="openCreate">+ Thêm cấp độ</AppButton>
    </template>
    <DataTable :columns="columns" :rows="items" :loading="loading" :total="items.length">
      <template #row="{ row }">
        <td class="px-5 py-4 font-semibold text-navy text-sm">{{ row.name }}</td>
        <td class="px-5 py-4"><AppBadge :variant="row.variant">{{ row.label }}</AppBadge></td>
        <td class="px-5 py-4 text-sm text-slate-500">{{ row.description }}</td>
        <td class="px-5 py-4">
    ...
    <AppModal v-model="showModal" :title="editItem ? 'Sửa cấp độ' : 'Thêm cấp độ'" size="sm">
      <form class="space-y-4">
        <AppInput v-model="form.name" label="Tên cấp độ" id="name" required />
    ...
    <ConfirmDialog v-model="showDelete" title="Xóa cấp độ" message="Bạn có chắc muốn xóa cấp độ này?" :loading="deleting" @confirm="handleDelete" />
  </AdminPageLayout>
</template>
...
async function handleSave() {
  ...
  if (editItem.value) {
    ...
    items.value.unshift({ id: Date.now(), ...form.value, label: form.value.name, variant: 'gray' })
  }
...
onMounted(async () => {
  ...
  items.value = [
    { id: 1, name: 'beginner',     label: 'Cơ bản',   variant: 'success', description: 'Dành cho người mới bắt đầu' },
    { id: 2, name: 'intermediate', label: 'Trung cấp', variant: 'warning', description: 'Dành cho người đã có nền tảng' },
    { id: 3, name: 'advanced',     label: 'Nâng cao', variant: 'danger',  description: 'Dành cho người học nâng cao' },
  ]
  loading.value = false
})
</script>
