    ...
    <div class="section-container max-w-3xl space-y-6">
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 class="mt-1 text-3xl font-semibold tracking-tight theme-text">
            Thông báo
            <span
              v-if="unread > 0"
              class="ml-2 inline-flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold text-white"
        ...
        <button
          v-if="unread > 0"
          @click="markAllRead"
          class="mark-all-btn self-start rounded-full px-4 py-2 text-sm font-semibold transition"
        >
      ...
      <div v-if="loading" class="space-y-3">
        ...
        >
          <div class="h-10 w-10 rounded-2xl skeleton shrink-0"></div>
          <div class="flex-1 space-y-2">
            <div class="h-4 rounded skeleton w-1/3"></div>
            <div class="h-3 rounded skeleton w-2/3"></div>
            <div class="h-3 rounded skeleton w-1/4 mt-1"></div>
          </div>
        </div>
      </div>

      <!-- Empty -->
      <div v-else-if="!notifications.length" class="theme-card rounded-[30px] p-16 text-center">
        <p class="text-5xl">🔔</p>
          ...
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-2">
                ...
                <span
                  v-if="!n.is_read"
                  class="shrink-0 h-2.5 w-2.5 rounded-full mt-1"
                  style="background: var(--accent)"
                ></span>
              </div>
              <p class="mt-1 text-sm theme-text-soft leading-relaxed">{{ n.message ?? n.body }}</p>
...
async function markAllRead() {
  ...
  await Promise.all(unreadItems.map(n => notificationService.markRead(n.id)))
  unreadItems.forEach(n => { n.is_read = true })
}
...
.mark-all-btn:hover { background: var(--surface-strong); }
