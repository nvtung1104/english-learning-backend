    ...
    <div class="section-container max-w-5xl space-y-6">
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 class="mt-1 text-3xl font-semibold tracking-tight theme-text">Tài liệu bài học</h1>
        </div>
      ...
      <div class="theme-card rounded-[24px] p-4 flex flex-col gap-3 sm:flex-row">
        <select v-model="lessonFilter" class="search-input flex-1 rounded-2xl px-4 py-2.5 text-sm outline-none">
          <option value="">Tất cả bài học</option>
          <option v-for="l in lessons" :key="l.id" :value="l.id">{{ l.title }}</option>
        </select>
        <select v-model="typeFilter" class="search-input rounded-2xl px-4 py-2.5 text-sm outline-none">
      ...
      <div class="theme-card rounded-[30px] overflow-hidden">
        ...
        <div v-else-if="!filtered.length" class="p-12 text-center">
          <p class="text-4xl">📎</p>
          <p class="mt-4 font-semibold theme-text">Chưa có tài liệu nào</p>
        </div>
        <table v-else class="w-full text-sm">
          <thead>
            <tr class="table-head">
              <th class="px-5 py-4 text-left font-semibold theme-text-muted">URL tài liệu</th>
          ...
          <tbody>
            <tr v-for="m in filtered" :key="m.id" class="table-row border-t" style="border-color: var(--border)">
              <td class="px-5 py-4 theme-text max-w-xs truncate">
                <a :href="m.file_url" target="_blank" class="hover:underline" style="color: var(--accent)">
                  {{ m.file_url }}
                </a>
              </td>
              <td class="px-5 py-4 hidden md:table-cell">
                <span class="rounded-full px-2 py-0.5 text-xs font-semibold bg-sky-100 text-sky-700 dark:bg-sky-900/50 dark:text-sky-300">
                  {{ m.type ?? 'file' }}
                </span>
              </td>
              <td class="px-5 py-4 theme-text-soft hidden lg:table-cell">{{ m.lesson?.title ?? '—' }}</td>
              <td class="px-5 py-4 text-right">
                <div class="flex justify-end gap-2">
                  <button @click="openEdit(m)" class="action-btn rounded-xl px-3 py-1.5 text-xs font-semibold">Sửa</button>
                  <button @click="deleteMaterial(m.id)" class="danger-btn rounded-xl px-3 py-1.5 text-xs font-semibold">Xóa</button>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div v-if="modal" class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(0,0,0,0.5)">
        <div class="w-full max-w-md rounded-[28px] p-6 space-y-4" style="background: var(--surface-strong); border: 1px solid var(--border)">
          <h2 class="text-xl font-semibold theme-text">{{ editing ? 'Sửa tài liệu' : 'Thêm tài liệu' }}</h2>
          ...
          <div class="form-group">
            <label class="form-label">File / URL <span class="text-red-500">*</span></label>
            <FileUploadField v-model="modalForm.file_url"
              :file-type="modalForm.type === 'audio' ? 'audio' : modalForm.type === 'video' ? 'video' : modalForm.type === 'pdf' || modalForm.type === 'doc' ? 'file' : 'file'"
              upload-folder="materials" />
          </div>
          ...
          <div class="flex gap-3 pt-1">
            <button @click="saveMaterial" :disabled="saving" class="submit-btn rounded-full px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-60">
              {{ saving ? 'Đang lưu...' : 'Lưu' }}
            </button>
...
import { materialService, lessonService } from '@/services/teacher.service'
...
// DB fields: lesson_id, file_url, type
const modalForm = ref({ lesson_id: '', file_url: '', type: '' })

async function load() {
  loading.value = true
  try {
    const [mRes, lRes] = await Promise.all([materialService.list(), lessonService.list()])
...
const filtered = computed(() =>
  materials.value.filter(m => {
    const matchLesson = !lessonFilter.value || String(m.lesson_id) === String(lessonFilter.value)
...
function openAdd() {
  editing.value = null
  modalForm.value = { lesson_id: '', file_url: '', type: '' }
  modalError.value = ''
  modal.value = true
}
...
function openEdit(m) {
  editing.value = m
  modalForm.value = { lesson_id: m.lesson_id, file_url: m.file_url, type: m.type ?? '' }
  modalError.value = ''
  modal.value = true
}
...
async function saveMaterial() {
  ...
  try {
    const payload = { lesson_id: modalForm.value.lesson_id, file_url: modalForm.value.file_url, type: modalForm.value.type || null }
    if (editing.value) {
      await materialService.update(editing.value.id, payload)
    } else {
...
async function deleteMaterial(id) {
  ...
  await materialService.destroy(id)
  await load()
}
</script>
