    ...
    <div class="section-container max-w-5xl space-y-6">
      ...
      <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h1 class="mt-1 text-3xl font-semibold tracking-tight theme-text">Quản lý ngữ pháp</h1>
        </div>
      ...
      <div class="theme-card rounded-[24px] p-4 flex flex-col gap-3 sm:flex-row">
        <input v-model="search" type="text" placeholder="Tìm chủ đề ngữ pháp..." class="search-input flex-1 rounded-2xl px-4 py-2.5 text-sm outline-none" />
      ...
      <div class="theme-card rounded-[30px] overflow-hidden">
        ...
        <div v-else-if="!filtered.length" class="p-12 text-center">
          ...
          <p class="mt-4 font-semibold theme-text">Chưa có chủ đề ngữ pháp nào</p>
        </div>
        <table v-else class="w-full text-sm">
          <thead>
            <tr class="table-head">
              <th class="px-5 py-4 text-left font-semibold theme-text-muted">Chủ đề</th>
          ...
          <tbody>
            <tr v-for="g in filtered" :key="g.id" class="table-row border-t" style="border-color: var(--border)">
              <td class="px-5 py-4 theme-text font-medium">{{ g.title }}</td>
              <td class="px-5 py-4 theme-text-soft hidden md:table-cell">{{ g.lesson?.title ?? '—' }}</td>
              <td class="px-5 py-4 text-right">
                <div class="flex justify-end gap-2">
                  <button @click="openEdit(g)" class="action-btn rounded-xl px-3 py-1.5 text-xs font-semibold">Sửa</button>
                  <button @click="deleteGrammar(g.id)" class="danger-btn rounded-xl px-3 py-1.5 text-xs font-semibold">Xóa</button>
                </div>
              </td>
            </tr>
          </tbody>
      ...
      <div v-if="modal" class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(0,0,0,0.5)">
        <div class="w-full max-w-lg rounded-[28px] p-6 space-y-4" style="background: var(--surface-strong); border: 1px solid var(--border)">
          <h2 class="text-xl font-semibold theme-text">{{ editing ? 'Sửa chủ đề' : 'Thêm chủ đề ngữ pháp' }}</h2>
          ...
          <div class="form-group">
            ...
            <input v-model="modalForm.title" type="text" class="form-input" required />
          </div>
          <div class="form-group">
            <label class="form-label">Giải thích (explanation)</label>
            <textarea v-model="modalForm.explanation" class="form-input" rows="5" placeholder="Giải thích cấu trúc ngữ pháp..."></textarea>
          </div>
          ...
          <div class="flex gap-3 pt-1">
            <button @click="saveGrammar" :disabled="saving" class="submit-btn rounded-full px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-60">
              {{ saving ? 'Đang lưu...' : 'Lưu' }}
            </button>
...
import { grammarService, lessonService } from '@/services/teacher.service'
...
// DB fields: lesson_id, title, explanation
const modalForm = ref({ lesson_id: '', title: '', explanation: '' })

async function load() {
  loading.value = true
  try {
    const [gRes, lRes] = await Promise.all([grammarService.list(), lessonService.list()])
    grammars.value = gRes.data?.data ?? gRes.data ?? []
...
const filtered = computed(() =>
  grammars.value.filter(g => {
    const matchSearch = g.title?.toLowerCase().includes(search.value.toLowerCase())
    const matchLesson = !lessonFilter.value || String(g.lesson_id) === String(lessonFilter.value)
...
function openAdd() {
  editing.value = null
  modalForm.value = { lesson_id: '', title: '', explanation: '' }
  modalError.value = ''
  modal.value = true
}
...
function openEdit(g) {
  editing.value = g
  modalForm.value = { lesson_id: g.lesson_id, title: g.title, explanation: g.explanation ?? '' }
  modalError.value = ''
  modal.value = true
}
...
async function saveGrammar() {
  ...
  try {
    const payload = {
      ...
      explanation: modalForm.value.explanation || null,
    }
    if (editing.value) {
      await grammarService.update(editing.value.id, payload)
    } else {
...
async function deleteGrammar(id) {
  ...
  await grammarService.destroy(id)
  await load()
}
</script>
